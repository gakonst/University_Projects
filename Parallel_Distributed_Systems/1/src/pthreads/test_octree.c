...
int main(int argc, char** argv){
  ...
  if (argc != 7) { // Check if the command line arguments are correct 
    printf("Usage: %s N thr dist pop rep P\n"
	   "where\n"
	   "N    : number of points\n"
  ...
  for(int it = 0; it<repeat; it++){
    ...
    compute_hash_codes(hash_codes, X, N, nbins, min, max, nthr); // compute the hash codes
    ...
    morton_encoding(morton_codes, hash_codes, N, maxlev, nthr); // computes the Morton codes of the particles
    ...
    int *currentThreads = (int*)malloc(sizeof(int));
    *currentThreads = 1;
    gettimeofday (&startwtime, NULL); 
    struct radix_args *args = malloc(sizeof (struct radix_args));
    args->morton_codes = morton_codes;
    args-> sorted_morton_codes = sorted_morton_codes;
    args->permutation_vector = permutation_vector;
    args-> level_record = level_record;
    args->index = index;
    args->N = N;
    args->population_threshold = population_threshold;
    args->sft=3*(maxlev-1);
    args->lv = 0;
    args->totalThreads=nthr;
    args->currentThreads=currentThreads;
    truncated_radix_sort(args);
...
/*    truncated_radix_sort(morton_codes, sorted_morton_codes, 
    ...
    // Data rearrangement
    data_rearrangement(Y, X, permutation_vector, N, nthr);
